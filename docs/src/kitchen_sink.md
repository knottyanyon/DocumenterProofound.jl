...
Reference page for every themed component this package adds on top of Documenter's
default theme — math environments and admonition styles. Adapted from Qritical.jl's
own kitchen-sink page, with the Qritical-specific glossary/citation examples stripped
out so this page only demonstrates the theme itself.
...
body content is regular Documenter markdown — LaTeX, cross-references, and
...
over all rank-$\nu$ matrices $B$, where $\|\cdot\|_F$ is [Definition 1](#ks-def-frobenius).
...
!!! design-note "Why a split-block pattern"
    Math environments use `@raw html` only for the wrapping `<div>`s, so the body stays
    regular Documenter markdown — LaTeX, cross-references, and nested admonitions all work.

!!! info "Note"
    Standard Documenter info admonition, recolored to the Julia blue.

!!! warning "Warning"
    Standard Documenter warning admonition (unchanged from the default theme).
